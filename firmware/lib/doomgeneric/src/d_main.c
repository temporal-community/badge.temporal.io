...
static const char *GetGameName(const char *gamename)
{
    ...
    for (i=0; i<arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            expanded_name = Z_Malloc(gamename_size, PU_STATIC, 0);
            version = G_VanillaVersionCode();
            M_snprintf(expanded_name, gamename_size, deh_sub,
                       version / 100, version % 100);

            while (expanded_name[0] != '\0' && isspace((int)expanded_name[0]))
            {
                memmove(expanded_name, expanded_name + 1, gamename_size - 1);
            }

            while (expanded_name[0] != '\0' && isspace((int)expanded_name[strlen(expanded_name)-1]))
            {
                expanded_name[strlen(expanded_name) - 1] = '\0';
            }
