...
#define V_ORIGWIDTH  320
#define V_ORIGHEIGHT 200
...
{ 
    ...
    if (destx < 0) { int d = -destx; srcx += d; width -= d; destx = 0; }
    if (desty < 0) { int d = -desty; srcy += d; height -= d; desty = 0; }
...
{ 
    ...
    int sx = x * SCREENWIDTH  / V_ORIGWIDTH;
    int sy = y * SCREENHEIGHT / V_ORIGHEIGHT;
    int sx_end = (x + pw) * SCREENWIDTH / V_ORIGWIDTH;

    int c0 = 0;
    if (sx < 0) { c0 = -sx; }
    int c1 = sx_end - sx;
    if (sx + c1 > SCREENWIDTH) c1 = SCREENWIDTH - sx;
    if (c0 >= c1 || sy >= SCREENHEIGHT) return;

    for (int c = c0; c < c1; c++)
    {
        int src_col = (c * V_ORIGWIDTH + SCREENWIDTH / 2) / SCREENWIDTH;
        ...
        {
            int ptop = column->topdelta;
            int plen = column->length;
            source = (byte *)column + 3;

            int oy = (y + ptop) * SCREENHEIGHT / V_ORIGHEIGHT;
            int oy_end = (y + ptop + plen) * SCREENHEIGHT / V_ORIGHEIGHT;
            int orows = oy_end - oy;

            int r0 = 0;
            if (oy < 0) { r0 = -oy; oy = 0; }
            int r1 = orows;
            if (oy + r1 - r0 > SCREENHEIGHT) r1 = SCREENHEIGHT - oy + r0;

            dest = dest_screen + oy * SCREENWIDTH + sx + c;
            for (int r = r0; r < r1; r++)
            {
                int src_row = r * V_ORIGHEIGHT / SCREENHEIGHT;
                if (src_row >= plen) src_row = plen - 1;
                *dest = source[src_row];
                dest += SCREENWIDTH;
            }
            column = (column_t *)((byte *)column + plen + 4);
        }
    }
}
...
void V_DrawPatchFlipped(int x, int y, patch_t *patch)
{
    ...
    for (int c = c0; c < c1; c++)
    {
        ...
        column = (column_t *)((byte *)patch + LONG(patch->columnofs[pw - 1 - src_col]));
...
{
#if (SCREENWIDTH == V_ORIGWIDTH && SCREENHEIGHT == V_ORIGHEIGHT)
    memcpy(dest_screen, raw, SCREENWIDTH * SCREENHEIGHT);
#else
    for (int oy = 0; oy < SCREENHEIGHT; oy++)
    {
        int sy = oy * V_ORIGHEIGHT / SCREENHEIGHT;
        byte *src_row = raw + sy * V_ORIGWIDTH;
        byte *dst_row = dest_screen + oy * SCREENWIDTH;
        for (int ox = 0; ox < SCREENWIDTH; ox++)
        {
            dst_row[ox] = src_row[ox * V_ORIGWIDTH / SCREENWIDTH];
        }
    }
#endif
}
