...
#ifdef ESP_PLATFORM
doom_file_t *save_stream;
#define fread(buf,sz,cnt,f)  doom_fread(buf,sz,cnt,f)
#define fwrite(buf,sz,cnt,f) doom_fwrite(buf,sz,cnt,f)
...
char *P_SaveGameFile(int slot)
{
    ...
    size_t needed = strlen(savegamedir) + 32;
    if (filename == NULL || needed > filename_size)
    {
        free(filename);
        filename_size = needed;
