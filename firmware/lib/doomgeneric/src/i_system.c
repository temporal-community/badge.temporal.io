...
#ifdef ESP_PLATFORM
#define DEFAULT_RAM 2 /* MiB — fits in 8 MB PSRAM alongside MicroPython */
#define MIN_RAM     2
#else
...
#ifdef ESP_PLATFORM
        zonemem = heap_caps_malloc(*size, MALLOC_CAP_SPIRAM);
#else
        zonemem = malloc(*size);
#endif
...
static char *EscapeShellString(const char *string)
{
    char *result;
    char *r;
    const char *s;
...
static int ZenityErrorBox(const char *message)
{
    int result;
