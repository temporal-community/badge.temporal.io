  ...
  patch_t*	c[] )
{
    ...
    int		bottom;
    boolean	fits = false;
    /* lnodes[] are authored for 320x200; scale to current framebuffer. */
    int		nodex = (int)((long)lnodes[wbs->epsd][n].x * SCREENWIDTH / 320);
    int		nodey = (int)((long)lnodes[wbs->epsd][n].y * SCREENHEIGHT / 200);

    i = 0;
    do
    {
	left = nodex - SHORT(c[i]->leftoffset);
	top = nodey - SHORT(c[i]->topoffset);
