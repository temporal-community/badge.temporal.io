...
#ifdef ESP_PLATFORM
	{
	    doom_file_t *df = doom_fopen(name, "rb");
	    if (df == NULL)
	    {
	    ...
	    doom_fread(&savegamestrings[i], 1, SAVESTRINGSIZE, df);
	    doom_fclose(df);
	    LoadMenu[i].status = 1;
	}
#else
...
void M_SaveSelect(int choice)
{
    ...
    if (!strcmp(savegamestrings[choice], EMPTYSTRING)) {
        M_snprintf(savegamestrings[choice], SAVESTRINGSIZE, "SAVE %d", choice + 1);
    }
    M_DoSave(choice);
    return;
#endif
...
void M_Init (void)
{
    ...
    // Shareware only has Episode 1; hide the rest so M_EPI2..4 lumps
    // aren't required.  Registered has three episodes.
    if (gamemode == shareware)
	EpiDef.numitems = 1;
    else if (gamemode == registered && EpiDef.numitems > 3)
	EpiDef.numitems = 3;
