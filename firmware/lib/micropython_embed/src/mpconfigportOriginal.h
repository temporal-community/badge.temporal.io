// MicroPython embed configuration for Replay badge (Arduino framework).
#pragma once
...
// Used by binascii.hexlify / unhexlify (off when ROM level is CORE).
...
// Align with generated genhdr/moduledefs.h.
#define MICROPY_PY_ERRNO ( 1 )
#define MICROPY_PY_CMATH ( 1 )
#define MICROPY_PY_UCTYPES ( 1 )
// Default MICROPY_CONFIG_ROM_LEVEL is CORE; these extmod modules need explicit enables so
// PlatformIO compiles mod*.c bodies to match MICROPY_REGISTERED_EXTENSIBLE_MODULES.
...
// ── Standard `os` module ────────────────────────────────────────────────────
// Backed by VFS — provides os.listdir, os.stat, os.statvfs, os.uname, etc.
#define MICROPY_PY_OS ( 1 )
#define MICROPY_PY_OS_UNAME ( 1 )
#define MICROPY_PY_OS_SYNC ( 1 )

// Step 1 (staged): bring in esp32 machine + wireless sources from ports/esp32.
// Keep this off by default so host-side QSTR generation remains stable.
...
#define MICROPY_PY_MACHINE_INCLUDEFILE      "port/modmachine_esp32_subset.c"
...
// Matches ports/esp32/mpconfigport.h — used by machine_signal.c if enabled and by ports/esp32/machine_pin.c.
