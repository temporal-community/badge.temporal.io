...
plus installable community apps from community_apps/
...
SKIP_DIR_NAMES = {'__pycache__', 'micropython_tests'}
SKIP_EXTENSIONS = {'.pyc', '.pyo', '.wad'}
# micropython_tests/ is dev-only (uploadfs picks it up but we don't bake or distribute it).
...
DEFAULT_COMMUNITY_RAW_BASE = (
    "https://raw.githubusercontent.com/"
    "temporal-community/badge.temporal.io/main/community_apps"
)
...
def generate_community_apps(files: list[dict], data_dir: Path,
    ...
    for f in files:
        ...
        if len(parts) >= 2:
            ...
            # Top-level files in community_apps are docs or
...
def generate(project_dir: Path, script_file: Path = None):
    ...
    # community_apps/ lives at the repo root so submissions are easy to review.
    # The generated community_apps.json is ignored build output and uploaded
    ...
    # 3) generated community_apps.json (with inlined per-app file lists).
    #
...
def mirror_files(src: Path, dst: Path) -> None:
    ...
    Mirror policy is broader than the manifest's: we keep `micropython_tests/` in
